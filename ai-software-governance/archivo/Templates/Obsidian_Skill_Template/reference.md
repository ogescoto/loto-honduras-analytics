...
Política y matriz de permisos: `09_AI/Codebase_And_Vault_MCP.md`. Resumen operativo:

| Tool | Servidor | Para qué |
|---|---|---|
| `obsidian_search` / `obsidian_context_pack` | obsidian-vault | Localizar notas por recuperación híbrida (consulta barata en tokens) |
| `obsidian_get_backlinks` / `obsidian_stale_notes` | obsidian-vault | Qué notas documentan una zona y cuáles quedaron obsoletas |
| `obsidian_create_note` / `obsidian_append_to_note` / `obsidian_frontmatter_set` | obsidian-vault (`--enable-write`, **exclusivo del experto**) | Escribir la bóveda vía MCP |
| `detect_changes` | codebase-memory | Diff → símbolos afectados con riesgo (paso previo del modo ACTUALIZACIÓN) |
| `trace_path` / `search_graph` | codebase-memory | Impacto: quién llama a qué; buscar símbolos en el grafo |
| `get_architecture` / `get_code_snippet` | codebase-memory | Visión estructural real del código; leer un símbolo concreto |
| `index_status` | codebase-memory | Frescura del grafo (si está viejo, repórtalo; no re-indexes tú) |

> Sin MCPs: todo lo anterior se hace con Read/Grep/Glob como siempre (degradación elegante).
> Prohibido: `manage_adr` del grafo (los ADR viven solo en `02_Arquitectura/adr/`).
